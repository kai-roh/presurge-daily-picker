...
## 3. 장중 모니터링 MVP — GitHub Actions

Daily picker가 만든 watchlist 중 최대 20개를 미국 프리/정규/포스트장 동안 감시하고, BUY/TAKE_PROFIT/SELL/CAUTION watch signal을 Telegram alert chat으로 보낸다.
...
### 3.2 GitHub Actions cron

`.github/workflows/intraday_monitor.yml`에서 실행된다. 로컬 launchd는 중복 실행 방지를 위해 unload 상태로 둔다.
